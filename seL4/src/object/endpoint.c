...
static int l_ep_ptr_get_queue(lua_State *L)
{
    tcb_queue_t queue = lua_touserdata(L,2);
    endpoint_t *epptr = lua_topointer(L,1);
    queue.head = (tcb_t*)endpoint_ptr_get_epQueue_head(epptr);
    queue.end = (tcb_t*)endpoint_ptr_get_epQueue_tail(epptr);

    lua_islightuserdata(L,queue);
    return 1;
}

static inline void
static int l_ep_ptr_set_queue(lua_State *L)
{   
    tcb_queue_t queue = lua_touserdata(L,2);
    endpoint_t *epptr = lua_topointer(L,1);
...
{"ep_ptr_get_queue",l_ep_ptr_get_queue},
{"ep_ptr_set_queue",l_ep_ptr_set_queue},
{NULL, NULL}
};
