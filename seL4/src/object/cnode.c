...
static int  l_decodeCNodeInvocation(lua_State *L)
{
    ...
    word_t invLabel = lua_tonumber(L,5); 
    word_t length = lua_tonumber(L,4);
    cap_t cap = lua_touserdata(L,3);
    extra_caps_t excaps = lua_tonumber(L,2); 
    word_t *buffer = lua_topointer(L,1);
    ...
    if (invLabel < CNodeRevoke || invLabel > CNodeSaveCaller) {
        ...
        lua_pushnumber(L,EXCEPTION_SYSCALL_ERROR);
        return 1;
    }
    ...
    if (lu_ret.status != EXCEPTION_NONE) {
        ...
        lua_pushnumber(L,lu_ret.status);
        return 1;
    }
    ...
    if (invLabel >= CNodeCopy && invLabel <= CNodeMutate) {
        ...
        case CNodeCopy:
            ...
            if (dc_ret.status != EXCEPTION_NONE) {
                ...
                lua_pushnumber(L,dc_ret.status);
                return 1;
            }
            newCap = dc_ret.cap;
            isMove = false;
        ...
        default:
            assert (0);
            lua_pushnumber(L,EXCEPTION_NONE);
            return 1;
        }
        ...
        if (isMove) {
            lua_pushnumber(L,invokeCNodeMove(newCap, srcSlot, destSlot));
            return 1;
        } else {
            lua_pushnumber(L,invokeCNodeInsert(newCap, srcSlot, destSlot));
            return 1;
        }
    }
    ...
    if (invLabel == CNodeRevoke) {
        setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
        lua_pushnumber(L,invokeCNodeRevoke(destSlot));
        return 1;
    }

    if (invLabel == CNodeDelete) {
        setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
        lua_pushnumber(L,invokeCNodeDelete(destSlot));
        return 1;
    }
    ...
    if (invLabel == CNodeSaveCaller) {
        ...
        setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
        lua_pushnumber(L,invokeCNodeSaveCaller(destSlot));
        return 1;
    }
    ...
    if (invLabel == CNodeCancelBadgedSends) {
        ...
        setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
        lua_pushnumber(L,invokeCNodeCancelBadgedSends(destCap));
        return 1;
    }
...
{"decodeCNodeInvocation",l_decodeCNodeInvocation},
{NULL, NULL}
};
